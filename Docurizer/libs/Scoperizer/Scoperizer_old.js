...
exports.RuleSet = function(scopeDefs) {
	...
	scopeDefs.list.forEach(function(scopeDef, index) {
		...
		if(scopeDef.start) {
			...
			if(subGroupCount > 0 
			&& (scopeDef.matchGroupScopes === undefined || scopeDef.matchGroupScopes.length != subGroupCount))
...
exports.RuleSet.prototype.computeSubScopes = function(scope, branchStack) {
	...
	if(scope.allowedSubScopes) {
		...
		scope.subScopes.forEach((sub)=> {
			...
			subsRegexString += `(`+ (sub.start.source ? sub.start.source : sub.start) + `)`;
		});
...
exports.RuleSet.prototype.removeBreakingScopes = function(file) {
	...
	if(THIS.anywhereScopes) {
		THIS.anywhereScopes.forEach((scope) => {
			...
			file.rawCode = file.rawCode.replace(new RegExp(scope.start, 'g'), '');
		});
	}
	console.log(file.rawCode);
}
...
exports.RuleSet.prototype.scopify = function(file, scopeInfo) {
	...
	// 	console.log("BLOCK", file.rawCode.substr(scopeChunk.startIndex, 14));
	...
	var rawCode = file.rawCode,
		...
		nextMatchRegex = thisScope.nextMatchRegex;
		...
		while(stopLoop !== true) {
			...
			while(matchID > regex.matchPoints.length-1) {
				if(regex.complete !== true) {
					var matches = regex.exec(rawCode);
			...
			if(noMatchesLeft == false) {
				...
				// 	console.log(rawCode.substr(targetIndex, 24));
	...
	var progressEndMatchPointID = function(soonestIndex, indexIsEnd) {
		if(thisScope != THIS.rootScope){ 
			...
			if(endMatch == undefined) {
				...
				console.error(`"`+rawCode.substr(scopeChunk.startIndex, 15)+`"`);
	...
	while(subScopesLeft) {
		...
		var match = nextMatchRegex.exec(rawCode);
